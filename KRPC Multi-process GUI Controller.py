...
def auto_pilot(flight_dict, vessel_dict, orbit_dict, body_dict, ksc_dict, gui_pipe, kill_flag):
...
##        print("heading diff: " + str(angle_diff))
    ...
    def calc_delta_v(target_radius): #https://krpc.github.io/krpc/tutorials/launch-into-orbit.html
        mu = body_dict["gravitational_parameter"]
        r = orbit_dict["apoapsis"]
        a1 = orbit_dict["semi_major_axis"]
        a2 = target_radius
    ...
    def calc_burn_time(delta_v): #https://krpc.github.io/krpc/tutorials/launch-into-orbit.html
        F = vessel_dict["available_thrust"]
        Isp = vessel_dict["specific_impulse"] * 9.82
        m0 = vessel_dict["mass"]
...
################################################
    ...
    def launch():
        ...
        time.sleep(2)
        print("Launching off of " + str(body_dict["name"]) + "!")
...
##                  except:
    ...
    def climb_to_orbit(offset):
        target_alt = body_dict["equatorial_radius"] + body_dict["atmosphere_depth"] + offset
        while(orbit_dict["apoapsis"] < target_alt and kill_flag.value == 0):
            time.sleep(0.1)
...
##            follow_heading(90)
    ...
    def circularize_orbit_to_apoapsis():
        delta_v = calc_delta_v(orbit_dict["apoapsis"])
        ...
        burn_ut = ksc_dict["ut"] + orbit_dict["time_to_apoapsis"] - (burn_time/2.)
        lead_time = 5
        conn.space_center.warp_to(burn_ut - lead_time)
        while orbit_dict["time_to_apoapsis"] - (burn_time/2.) > 0 and kill_flag.value == 0:
            time.sleep(0.1)
    ...
    def hohmann_transfer(body): #https://github.com/whatdamath/KerbalSpaceProgram/blob/master/kerbalMoonTransfer.py
        ...
        angle = 10
        angular_diff = 15
        orbit_angular_velocity = 2*math.pi/orbit_dict["period"] #Angular velocity in rad/sec
        ...
        while True and kill_flag.value == 0:
            ...
            angular_diff = math.pi*((1-(1/(2*math.sqrt(2)))*math.sqrt((orbit_dict["semi_major_axis"]/target_radius+1)**3)))
...
def spam_science(stream_flight_dict, stream_vessel_dict, gui_pipe, kill_flag):
    def run_exp():
    ...
    while(kill_flag.value == 0):
        ...
        if(science_on):
            if(stream_vessel_dict["biome"] != prev_biome):
                print("Transition biome: " + str(prev_biome) + " to " + stream_vessel_dict["biome"])
                prev_biome = stream_vessel_dict["biome"]
                run_exp()
...
def GUI(stream_flight_dict, stream_vessel_dict, stream_orbit_dict, stream_body_dict, stream_ksc_dict):
        ...
        autopilot_process = Process(target=auto_pilot, args=(stream_flight_dict, stream_vessel_dict, stream_orbit_dict, stream_body_dict, stream_ksc_dict, auto_to_gui, kill_flag))
        science_process = Process(target=spam_science, args=(stream_flight_dict, stream_vessel_dict, science_to_gui, kill_flag))
...
def main():
    with Manager() as manager: #Use manager to allow objects such as dictionaries to be shared between processes - slower than shared memory but allows more complex data structures
        stream_flight_dict = manager.dict()
        stream_vessel_dict = manager.dict()
        stream_orbit_dict = manager.dict()
        stream_body_dict = manager.dict()
        stream_ksc_dict = manager.dict()
        ...
        stream_flight_dict["velocity"] = manager.list([0]*3) #The velocity of the vessel, in the reference frame ReferenceFrame.
        ...
        stream_flight_dict["center_of_mass"] = manager.list([0]*3) #The position of the center of mass of the vessel, in the reference frame ReferenceFrame
        stream_flight_dict["rotation"] = manager.list([0]*4) #The rotation of the vessel, in the reference frame ReferenceFrame
        stream_flight_dict["direction"] = manager.list([0]*3) #The direction that the vessel is pointing in, in the reference frame ReferenceFrame.
        ...
        stream_flight_dict["prograde"] = manager.list([0]*3) #The prograde direction of the vessels orbit, in the reference frame ReferenceFrame.
        stream_flight_dict["retrograde"] = manager.list([0]*3) #The retrograde direction of the vessels orbit, in the reference frame ReferenceFrame.
        stream_flight_dict["normal"] = manager.list([0]*3) #The direction normal to the vessels orbit, in the reference frame ReferenceFrame.
        stream_flight_dict["anti_normal"] = manager.list([0]*3) #The direction opposite to the normal of the vessels orbit, in the reference frame ReferenceFrame.
        stream_flight_dict["radial"] = manager.list([0]*3) #The radial direction of the vessels orbit, in the reference frame ReferenceFrame.
        stream_flight_dict["anti_radial"] = manager.list([0]*3) #The direction opposite to the radial direction of the vessels orbit, in the reference frame ReferenceFrame.
        ...
        stream_flight_dict["aerodynamic_force"] = manager.list([0]*3) #The total aerodynamic forces acting on the vessel, in reference frame ReferenceFrame.
        stream_flight_dict["lift"] = manager.list([0]*3) #The aerodynamic lift currently acting on the vessel.
        stream_flight_dict["drag"] = manager.list([0]*3) #The aerodynamic drag currently acting on the vessel.
        ...
        stream_vessel_dict["available_thrust"] = 0.0 #Gets the total available thrust that can be produced by the vessel’s active engines, in Newtons. This is computed by summing Engine.AvailableThrust for every active engine in the vessel.
